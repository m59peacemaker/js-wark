...
test('value remains the same until the subsequent instant of its update', () => {
	...
	assert.equal(b.perform(), 0)
	assert.equal(c.perform(), 1)
	a.produce(1)
	assert.equal(b.perform(), 1)
	assert.equal(c.perform(), 2)
	a.produce(2)
	assert.equal(b.perform(), 2)
	assert.equal(c.perform(), 3)
	a.produce(3)
	assert.equal(b.perform(), 3)
	assert.equal(c.perform(), 4)
	assert.equal(values_c_updates, [ 2, 3, 4 ])
	assert.equal(values_d, [ 1, 2, 3 ])
})

test(
	'computes only when value is checked when update event is not observed',
	() => {
		const values = []
		const a = Event.create()
		const b = Event.hold (0) (a)
...
test(
	'computes when update event occurs (eager,push), not when checked (lazy,pull), when update event is observed',
	() => {
		const values = []
		const a = Event.create()
		const b = Event.hold (0) (a)
		...
		const e = Event.tag (c) (d)
		Event.calling (x => values.push(x)) (e)
		Event.calling (() => {}) (Dynamic.updates (c))
		a.produce(1)
		assert.equal(calls, 1)
		d.produce('_')
		assert.equal(calls, 1)
