...
test('ending a stream', t => {
  ...
  /* when one value enters the system, the node should only ever be notified one time
    with its final state, so, there can't be an update AND an end from one notification
    I guess?
    the big question is: can/should a node be updated if it is also going to end in this cycle?
    */
  t.test(`if a stream's dependencies and its end stream's dependencies both have changed within the same update cycle, do not recompute the stream, only end it`, t => {
    const x = Stream (3)
